...
export default function UserAccessAdminPage() {
  ...
  const [saveLoading, setSaveLoading] = useState<number | null>(null); // Track which user is being saved
  const [deleteLoading, setDeleteLoading] = useState<number | null>(null); // Track which user is being deleted
  const [confirmDelete, setConfirmDelete] = useState<number | null>(null); // Track which user is pending confirmation
  const [error, setError] = useState("");
  const [success, setSuccess] = useState("");
  const [pocOptions, setPocOptions] = useState<PocOption[]>([]);
  const [tzMap, setTzMap] = useState<Record<number, string>>({}); // userId -> tz
  ...
  const handleSave = async (user: User) => {
    ...
    const tz = tzMap[user.id] || "America/New_York";
  ...
  const filteredUsers = users.filter(user => {
    ...
    const matchesRoleFilter = !activeFilters.role || user.role === activeFilters.role;
              ...
              <tr
                ...
                className={
                  ...
                  <select
                    ...
                    onChange={(e) => handleChange(user.id, "accessLevel", e.target.value)}
                  >
                    {ACCESS_LEVELS.map((a) => (
                  ...
                  <div className="flex items-center gap-1">
                    <input
                      ...
                      value={convertUTCToLocal(user.accessStartTime, tzMap[user.id])}
                      onChange={(e) => handleChange(user.id, "accessStartTime", e.target.value)}
                    />
                    ...
                    <input
                      ...
                      value={convertUTCToLocal(user.accessEndTime, tzMap[user.id])}
                      onChange={(e) => handleChange(user.id, "accessEndTime", e.target.value)}
                    />
                    <Select
                      options={TIMEZONES}
                      isDisabled={user.accessLevel !== "partial"}
                      value={TIMEZONES.find((tz) => tz.value === tzMap[user.id])}
                      onChange={(opt) => setTzMap((prev) => ({ ...prev, [user.id]: opt?.value || "America/New_York" }))}
                  ...
                  <Select
                    ...
                    onChange={(selected) => {
                      ...
                      handleChange(user.id, "pageAccess", newPageAccess);
                    }}
                    classNamePrefix="react-select"
                  ...
                  <button
                          className={`btn-ai py-1.5 px-4 text-sm flex items-center gap-1 ${saveLoading === user.id ? 'opacity-70 cursor-not-allowed' : ''}`}
                  ...
                  <button
                    className={`${
                      ...
                      deleteLoading === user.id ? 'opacity-70 cursor-not-allowed' : ''
                    }`}
