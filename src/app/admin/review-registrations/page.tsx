...
export default function ReviewRegistrations() {
  ...
  useEffect(() => {
    ...
    const fetchPendingUsers = async () => {
      try {
      const response = await fetch('/api/admin/getPendingUsers');
        if (!response.ok) throw new Error('Failed to fetch pending users');
      const data = await response.json();
      setUsers(data);
      } catch (error) {
        setMessage({ type: 'error', text: 'Failed to load pending registrations' });
      } finally {
  ...
  const handleApproval = async (userId: number, status: string) => {
    try {
      setMessage({ type: '', text: '' });
      const response = await fetch(`/api/admin/updateUserStatus`, {
      method: 'POST',
      headers: {
