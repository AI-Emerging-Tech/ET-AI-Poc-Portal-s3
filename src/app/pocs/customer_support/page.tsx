...
export default async function CustomerSupportPage() {
  // Path to the customer_support directory
  const customer_supportDir = path.join(process.cwd(), 'src', 'app', 'pocs', 'customer_support');

  // Get the list of files/folders in the customer_support directory
  const items = await fs.readdir(customer_supportDir);
  ...
  const poCs = await Promise.all(directories.map(async (directory) => {
    const dirPath = path.join(customer_supportDir, directory);
    ...
    <div className="min-h-screen bg-light-gray relative">
      ...
      <section className="relative bg-white py-12 mb-8">
        ...
        <div className="container max-w-ultra mx-auto px-6 relative z-10">
          <h1 className="text-4xl font-bold mb-4 text-gradient">Customer Service PoCs</h1>
          <p className="text-medium-gray max-w-3xl mb-8">
            Explore innovative solutions for enhancing customer service experiences.
          </p>
        </div>
      </section>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {poCs.map(({ directory, title, description }) => (
                <div key={directory} className="bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow duration-normal border-t-2 border-gradient overflow-hidden">
                  <Link href={`/pocs/customer_support/${directory}`} className="block p-6">
