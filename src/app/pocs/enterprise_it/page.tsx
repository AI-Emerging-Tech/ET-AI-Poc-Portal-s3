...
export default async function EnterpriseItPage() {
  // Path to the enterpriseIt directory
  const enterpriseItDir = path.join(process.cwd(), 'src', 'app', 'pocs', 'enterprise_it');

  // Get the list of files/folders in the enterpriseIt directory
  const items = await fs.readdir(enterpriseItDir);
  ...
  const poCs = await Promise.all(directories.map(async (directory) => {
    const dirPath = path.join(enterpriseItDir, directory);
    ...
    <div className="min-h-screen bg-light-gray relative">
      ...
      <section className="relative bg-white py-12 mb-8">
        ...
        <div className="container max-w-ultra mx-auto px-6 relative z-10">
          <h1 className="text-4xl font-bold mb-4 text-gradient">Enterprise IT PoCs</h1>
          <p className="text-medium-gray max-w-3xl mb-8">
            Explore innovative solutions for enterprise IT operations and infrastructure.
          </p>
        </div>
      </section>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {poCs.map(({ directory, title, description }) => (
                <div key={directory} className="bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow duration-normal border-t-2 border-gradient overflow-hidden">
                  <Link href={`/pocs/enterprise_it/${directory}`} className="block p-6">
